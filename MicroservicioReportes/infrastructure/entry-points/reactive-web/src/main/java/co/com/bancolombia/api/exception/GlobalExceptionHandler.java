...
import co.com.bancolombia.api.exception.models.ErrorCode;
import co.com.bancolombia.api.exception.models.ValidationError;
import co.com.bancolombia.api.exception.models.Validations;
...
private Map<String, Object> toBody(Throwable ex, HttpStatus status) {
    ...
    if (ex instanceof amountNotValid exc ) {
        body.put(fieldErrors, this.MessageErrorToMap(exc.getMessage(), exc.getField()));
        body.put(code, ErrorCode.DUPLICATE.name());
        return body;
    }

    if (ex instanceof approvedDataRequired exc) {
        body.put(fieldErrors, this.MessageErrorToMap(exc.getMessage(), exc.getField()));
        body.put(code, ErrorCode.FORBIDDEN.name());
        return body;
    }

    if (ex instanceof getReportException exc) {
        body.put(fieldErrors, this.MessageErrorToMap(exc.getMessage(), exc.getField()));
        body.put(code, ErrorCode.NOT_FOUND.name());
        return body;
    }

    if (ex instanceof loanIdRequired exc) {
        body.put(fieldErrors, this.MessageErrorToMap(exc.getMessage(), exc.getField()));
        body.put(code, ErrorCode.FORBIDDEN.name());
        return body;
    }

    if (ex instanceof reportNotFound exc) {
        body.put(fieldErrors, this.MessageErrorToMap(exc.getMessage(), exc.getField()));
        body.put(code, ErrorCode.NOT_FOUND.name());
        return body;
    }
